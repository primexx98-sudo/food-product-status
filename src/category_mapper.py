...
# '전'(부침개류)은 2026-09-17 AI 스크리닝(기타 3,699건 전수 감사)으로 추가 — 기존 카테고리
# 배정 품목 중 '전'으로 끝나는 것이 0건임을 재검증 후 반영(부작용 없음 확인)
_GENERAL_EXCLUDE_SUFFIXES = ('청', '소스', '양념', '드레싱', '나물', '찜', '전')
...
_GENERAL_EXCLUDE_CONTAINS = (
    ...
    # 2026-09-17 AI 스크리닝(기타 3,699건 전수 감사, 사용자 확인 후 반영) — 아래 전부
    # 현재 분류된(기타 아닌) 품목명과 충돌 0건 재검증 완료
    '필링',             # 에그타르트필링/솔티드카라멜필링 등 베이커리·외식 B2B 충전재
    '고로케',           # 고로케 속재료/감자고로케 등 튀김 스낵
    '슬라이스',         # 마늘/배추/애호박 슬라이스 등 원물 단순 절단 가공(냉동 규칙과 동일 성격)
    '구슬식초',         # 석류/파인애플/매실/사과 구슬식초 — 기능성 없는 맛과일 식초캔디
    '콜리플라워라이스', # 볶아먹는 김치맛/카레볶음 콜리플라워라이스 등 조리용 반가공 채소밥
    '치킨파우더', '치킨용파우더', '가라아게', '닭강정파우더',  # 튀김닭용 B2B 코팅파우더
    '팝핑캔디', '팝핑 캔디',  # 기호 캔디
    '탕후루',           # 기호 간식
    '떡볶이파우더',     # 기존 떡볶이분말/떡볶이파우다 표기 변형 누락분
    '뽀로로', '짱구', '산리오', '배드바츠마루', '커비', 'BT21',  # 아동 캐릭터 라이센스 캔디·젤리
    '초코믹스', '치즈파우더',  # 카페·베이커리용 B2B 향미파우더(컴포즈치즈파우더/아이스크림용 초코믹스 등)
)
...
_GENERAL_EXCLUDE_COMPANIES = {
    ...
    # 2026-09-17 AI 스크리닝(기타 3,699건 전수 감사, 사용자 확인 후 반영) — 위 키워드
    # 규칙으로 못 잡는 개별 품목명(프리믹스/배터믹스 단독 표기 등)까지 포함해 회사 단위로 제외
    '주식회사 광진푸드',   # 뚝배기불고기/연포탕/아귀불고기/조림/마라탕 등 조리 밀키트 8종 전량
    '(농)농가생활협동조합',  # IQF 얼갈이·근대·시금치, 무말랭이, 깐마늘 등 원물손질·냉동채소 전량
    '(주)바인푸드',        # 핫치킨파우더/프리믹스/배터믹스 등 B2B 튀김코팅재 전량
    '(주)네오푸드',        # 청경채/애호박/셀러리/브로콜리 등 가공·브랜딩 없는 생채소 품목명만
}
...
_GENERAL_EXCLUDE_REPORT_NOS = {
    ...
    # 2026-09-17 AI 스크리닝(기타 3,699건 전수 감사, 사용자 확인 후 반영) — 이름 패턴으로
    # 일반화 불가능한 개별 품목
    '20140560084127',  # 도토리 담은 발효식초 ((주)들산초) - 발효식초 자체는 수집대상이라 키워드 배제 대신 개별 처리
    '202605273108',    # 굴림곶감 (현대무브 주식회사) - 전통 곶감 단순가공
}
