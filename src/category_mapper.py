...
_HEALTH_MAP = [
    ...
    # 비타민은 맨 마지막 — 필수영양소 설명 문구("~에 필요")도 여기서 포괄
...
_GENERAL_EXCLUDE_SUFFIXES = ('청', '소스', '시즈닝', '양념', '드레싱', '나물')


def is_general_excluded(product_name: str) -> bool:
    name = str(product_name or '').strip()
    return any(name.endswith(s) for s in _GENERAL_EXCLUDE_SUFFIXES)
...
def categorize_health_food(fnclty_cn: str, product_name: str = '', raw_material: str = '') -> str:
    ...
    # 3. 비타민/미네랄만 있는 경우 → 즉시 비타민
    #    단, 기능성 텍스트에 뼈건강 명시된 경우는 뼈건강 우선
